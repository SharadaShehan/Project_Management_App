...
import { createRequest, respondRequest } from '../schemas/request.js'
...
export default {
  ...
  Mutation: {
    createRequest: async (root, args, { req }, info) => {
      ...
      if (project.members.includes(args.receiver)) {
    ...
    respondRequest: async (root, args, { req }, info) => {
      ...
      if (project.members.includes(request.receiver)) {
        throw new Error('You are already in this project')
      }
    ...
    cancelRequest: async (root, args, { req }, info) => {
      Auth.checkSignedIn(req)
      const request = await Request.findOne({ _id: args.id })
      if (!request) {
        throw new Error('Request not found')
      }
      if (request.sender.toString() !== req.session.userId) {
        throw new Error('Unauthorized')
      }
  ...
  Request: {
    ...
    sender: async (request, args, { req }, info) => {
      return (await request.populate('sender')).sender
    },
